...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    ...
    try {
      ...
      await multi.ZADD('post', { score: parseInt(uId, 10), value: `${key}` });
  ...
  public async getUserPostsFromCache(key: string, uId: number): Promise<IPostDocument[]> {
    try {
      ...
      const scores = await Promise.all(reply.map(async (value) => Number(await this.client.ZSCORE(key, value))));

      const customSort = (a: string, b: string) => scores[reply.indexOf(b)] - scores[reply.indexOf(a)];

      const sortedResponse = reply.sort(customSort).reverse();

      const multi: ReturnType<typeof this.client.multi> = this.client.multi();
      for (const value of sortedResponse) {
